...
void outputAll(ostream& fout, double hpwl, std::vector<Block*>& softBlockList);
...
int main(int argc, char* argv[]) {
    ...
    int TOTAL_TRIALS = 500;
    for ( int trialIndex = 0; trialIndex < TOTAL_TRIALS; trialIndex++ ) {
        std::cout << "Trial: " << trialIndex << '/' << TOTAL_TRIALS << '\n';
        ...
        soft_modules_vector.clear();
        fixed_modules_vector.clear();
        connections_vector.clear();
        allblock_vector.clear();

        fin >> readstr >> CHIP_WIDTH >> CHIP_HEIGHT;
        // std::cout << "readstr: " << readstr << endl;
        // std::cout << "cw: " << CHIP_WIDTH << endl;
        // std::cout << "ch: " << CHIP_HEIGHT << endl;
        ...
        //     std::cout << soft_modules_vector[i]->blockname;
        ...
        //     std::cout << "[" << target->blockname <<", "<<target->id<<"]" << target->block_x <<" " <<  target->block_y << " ";
        //     std::cout << target->width << " " <<target->height <<endl;
        // }

        fin >> readstr >> NUMBER_OF_CONNECTIONS;
        // std::cout << "connect: " << NUMBER_OF_CONNECTIONS<<endl;
        int found_node;
        ...
        //     std::cout << "[" << connections_vector[i]->first->blockname << "]" << "--- ";
        //     std::cout << connections_vector[i]->num_nets << " --->";
        //     std::cout << "[" << connections_vector[i]->second->blockname << "]" <<endl;
        // }

        /*Start pushing blocks into B*-Tree* and initialize B*-Tree */
        BStarTree BST;
        int floorplan_width, floorplan_height;
        int newChipWidth, newChipHeight;
        ...
        BST.noPPMInit(allblock_vector, CHIP_WIDTH, CHIP_HEIGHT, &newChipWidth, &newChipHeight);
        R_Star = newChipHeight / newChipWidth;

        BST.saveCurrent();

        std::string filename1 = "initFloorplan.txt";
        BST.printFloorplan(filename1, CHIP_WIDTH, CHIP_HEIGHT, connections_vector);
        ...
        for ( int init_peturb_idx = 0; init_peturb_idx < initial_peturb_rounds; init_peturb_idx++ ) {
            ...
            while ( success == 0 ) {
                ...
                case 0:     // rotate
                    ...
                    // std::cout <<"Rotate SB" << target0 <<endl;
                    break;
                case 1:     // resize
            ...
            // std::cout <<"["<< init_peturb_idx << "]";
            // std::cout <<"L:"<<BST.getisLegal()<<",W: "<<floorplan_width <<",H: "<<floorplan_height << ",A: "<< floorplan_width*floorplan_height << ", HPWL: ";
            // std::cout <<  CalculateConnection(connections_vector, NUMBER_OF_CONNECTIONS)<<endl; 
        }
        ...
        // std::cout << "Statistics collect phase run " << initial_peturb_rounds <<  " peturbs, uphill: " << init_peturb_uphill_count <<endl;
        // std::cout << "A_norm: " << A_norm <<endl;
        // std::cout << "W_norm: " << W_norm <<endl;    
        // std::cout << "RtoRstar_norm: " << RtoRstar_norm <<endl;
        // std::cout <<"Delta_cost_avg: " << Delta_cost_avg <<endl;
        // std::cout << "Delta_uphill_avg: " << Delta_uphill_avg <<endl;
        ...
        // std::cout << "[    0] \t";
        // std::cout <<"L:"<<BST.getisLegal()<<",W: "<<floorplan_width <<",H: "<<floorplan_height << ",A: "<< sa_peturb_area << ", HPWL: ";
        // std::cout <<  sa_petrub_wirelength;
        // std::cout <<",Cost: "<< sa_peturb_cost <<endl;

        for ( int sa_n = 0; sa_n < ( 1 + SA_RUN_PHASE2_RNDS + SA_RUN_PHASE3_RNDS ); sa_n++ ) {
            // for(int sa_n = 0; sa_n  < 0; sa_n++){

                // if(sa_n < 1) std::cout <<"P1";
                // else if(sa_n < (1+SA_RUN_PHASE2_RNDS)) std::cout <<"P2";
                // else std::cout <<"P3";
                // printf("[%5d] \t\n",sa_n + 1);
            ...
            for ( int sa_pt = 0; sa_pt < NUMBER_OF_SOFT_MODULES * SA_PERTURB_PER_BLOCK; sa_pt++ ) {
                ...
                if ( sa_peturb_cost < sa_peturb_last_cost ) { // this is a downhill movement, accept!
                    ...
                    // std::cout <<"L:"<<BST.getisLegal()<<",W: "<<floorplan_width <<",H: "<<floorplan_height << ",A: "<< floorplan_width*floorplan_height << ", HPWL: ";
                    // std::cout <<  sa_petrub_wirelength;
                    // std::cout <<",Cost: "<< sa_peturb_cost;
                    // // std::cout << ", Temp: " << SA_temperature;
                    // std::cout << endl;
                    BST.saveCurrent();
                }
                    ...
                    // std::cout <<" *UPHILL*";
                    // std::cout <<" Cdiff: +"<<sa_peturb_cost_diff;
                    // std::cout << ", Temp: " << SA_temperature;
                    // std::cout << ", Prob: "<< sa_probability;
                    // std::cout << ", Accept: " << sa_uphill <<endl;
            ...
            //adjust temperature
            // std::cout << "ADJUSTING TEMP: " << sa_delta_cost_avg << '\n';
            if ( sa_n < ( 1 + SA_RUN_PHASE2_RNDS ) ) { // At Phase 2
                // std::cout << "absdouble sapetb: " << (absDouble(sa_peturb_cost_diff)*0.01 + 0.99) <<endl;
                // std::cout << "noabs sapetb: " << sa_peturb_cost_diff <<endl;
        ...
        if ( floorplan_width <= newChipWidth && floorplan_height <= newChipHeight ) {
            std::cout << "legal floorplan found\n";
            outputAll(fout, sa_petrub_wirelength, soft_modules_vector);
            std::cout << "HPWL: " << sa_petrub_wirelength << std::endl;
            std::string filename2 = "legalFloorplan.txt";
            BST.printFloorplan(filename2, CHIP_WIDTH, CHIP_HEIGHT, connections_vector);
            legalFound = true;
            break;
        }

        // std::string filename2 = "finalFloorplanIter" + std::to_string(trialIndex) + ".txt";
        // BST.printFloorplan(filename2, CHIP_WIDTH, CHIP_HEIGHT);

        if ( trialIndex == TOTAL_TRIALS - 1 ) {
            std::cout << "FAIL to find legal floorplan\n";
            //final print
            outputAll(fout, sa_petrub_wirelength, soft_modules_vector);
            std::string filename2 = "legalFloorplan.txt";
            BST.printFloorplan(filename2, CHIP_WIDTH, CHIP_HEIGHT, connections_vector);
...
void printBlock(Block* block) {
    ...
    std::cout << setw(8) << block->blockname << "(" << block->id << ")]";
...
void outputAll(ostream& fout, double hpwl, std::vector<Block*>& softBlockList) {
    fout << "HPWL ";
    fout << std::fixed;
    fout << std::setprecision(2) << hpwl;
    fout << "\nSOFTMODULE " << softBlockList.size() << '\n';
    for ( int softIndex = 0; softIndex < softBlockList.size(); softIndex++ ) {
        Block* currentBlock = softBlockList[softIndex];
        fout << currentBlock->blockname << " 4\n";
        fout << currentBlock->block_x << ' ' << currentBlock->block_y << '\n';
        fout << currentBlock->block_x << ' ' << currentBlock->block_y + currentBlock->height << '\n';
        fout << currentBlock->block_x + currentBlock->width << ' ' << currentBlock->block_y + currentBlock->height << '\n';
        fout << currentBlock->block_x + currentBlock->width << ' ' << currentBlock->block_y << '\n';
    }
}